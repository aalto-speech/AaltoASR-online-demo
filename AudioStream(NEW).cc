...
{
  m_stream = NULL;
  m_input_stream = false;
  m_output_stream = false;
  m_input_buffer = NULL;
  m_output_buffer = NULL;
  pthread_mutex_init(&m_inputbuffer_lock, NULL);
  pthread_mutex_init(&m_outputbuffer_lock, NULL);
//  m_frames_played = 0;
}
...
{
  close();
  pthread_mutex_destroy(&m_inputbuffer_lock);
  pthread_mutex_destroy(&m_outputbuffer_lock);
}
...
PaStream* AudioStream::m_stream = NULL;
bool AudioStream::m_input_stream = false;
bool AudioStream::m_output_stream = false;
AudioBuffer* AudioStream::m_input_buffer = NULL;
AudioBuffer* AudioStream::m_output_buffer = NULL;
pthread_mutex_t AudioStream::m_inputbuffer_lock = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t AudioStream::m_outputbuffer_lock = PTHREAD_MUTEX_INITIALIZER;
...
{
  pthread_mutex_lock(&m_inputbuffer_lock);
  m_input_buffer = input_buffer;
  pthread_mutex_unlock(&m_inputbuffer_lock);
}
...
//    Pa_StartStream(m_stream);
  pthread_mutex_lock(&m_outputbuffer_lock);
  m_output_buffer = output_buffer;
  pthread_mutex_unlock(&m_outputbuffer_lock);
}
...
//  AudioStream *object = (AudioStream*)instance;
  if (m_input_stream) {
    input_stream_callback((AUDIO_FORMAT*)input_buffer,
                          frame_count);
  }
//  memcpy(output_buffer, input_buffer, frame_count * sizeof(AUDIO_FORMAT));
  //*
  if (m_output_stream) {
    output_stream_callback((AUDIO_FORMAT*)output_buffer,
                           frame_count);
  }
  //*/
//  Pa_Sleep(1);
                                   ...
                                   unsigned long frame_count)
{
  ...
  pthread_mutex_lock(&m_inputbuffer_lock);
  if (m_input_buffer) {
    write_size = m_input_buffer->write(input_buffer, frame_count);
  
    if (write_size < frame_count) {
      fprintf(stderr, "Warning: Audio input buffer full, losing audio.\n");
      assert(false);
    }
                                    ...
                                    unsigned long frame_count)
{
  ...
  pthread_mutex_lock(&m_outputbuffer_lock);
  if (m_output_buffer) {
    read_size = m_output_buffer->read(output_buffer, frame_count);
  }
  else {
    read_size = 0;
  }
  ...
  if (frame_count > read_size) {
    unsigned long empty_size = sizeof(AUDIO_FORMAT) * (frame_count - read_size);
    memset(output_buffer + read_size, 0, empty_size);
  }
